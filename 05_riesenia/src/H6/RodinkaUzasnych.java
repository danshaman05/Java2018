...
class Person {
	...
	public List<Person> fathersOfMyChild() {
		...
		.stream()
		.filter(child -> child != null && child.parents != null && child.parents.contains(this))
		.flatMap(child -> child.parents.stream())
		.filter(partner -> partner.sex == Sex.Male)
		.filter(father -> !father.equals(this))
		.distinct()
	...
	//ave a child with this and it's a woman
    ...
    public Set<Person> predecessors() {
    	...
    	,
        parents.stream()
    	.flatMap(parent -> parent.predecessors().stream())
    	).distinct()
