...
class Person {
    ...
    public boolean isSib(Person y) {
    	return this.parents != null && this.parents
				.stream()
				.filter(person -> y.parents != null && y.parents.contains(person))
				.count() > 0;
    ...
    public boolean isSister(Person y) {
		return this.isSib(y) && this.sex == Sex.Female;
    }
    ...
    public List<Person> partners() {
		...
		RodinkaUzasnych.people
				.stream()
				.filter(child->child != null && child.parents.contains(this))
				.flatMap(child-> child.parents.stream());
...
public class RodinkaUzasnych {
	...
	public static void main(String[] args) {
		System.out.println(jozo.mothersOfMyChild());
	}
}
