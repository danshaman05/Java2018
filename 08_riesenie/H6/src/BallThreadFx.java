...
class BallThreadPanel extends Pane {
	...
	protected void paintBallPane() {
		getChildren().clear();
		if (blue != null) {
			Circle blueR = new Circle(Math.round(blue.x), Math.round(blue.y), blue.size);
			blueR.setFill(Color.BLUE);
			blueR.setStroke(Color.BLACK);
			getChildren().add(blueR);
		}
		if (red != null) {
			Circle redR = new Circle(Math.round(red.x), Math.round(red.y), red.size);
