...
public class Minesweeper extends Applet implements Runnable {
  ...
  /* "exposed" contains the exposure state of the board.  Values > 
"unexposed"
     ...
     list through this array, using array indexes.  The method holds the 
head
  ...
  /* Various state used in painting.  There are offscreen images for 
each state
     of the squares */
  ...
  private synchronized void erase() {
    ...
    while (laid < mines) {
        ...
        if (target>=0 && target < height*width && adjacent[target] != 
mine) {
...
count,
  ...
  private void paintNumber(Graphics g, int n, int right) {
    ...
    g.setColor(Color.black);
    g.fillRect(right-numberSize, top-numberMargin, numberSize, 
fontHeight+numberMargin*2);
  ...
  private void paintFlags(Graphics g) {
    paintNumber(g, mines-flags, 
numberSize+(width*edge-faceSize-numberSize*2)/4);
  }
