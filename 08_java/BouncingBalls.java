...
public class BouncingBalls extends Application {
				...
				new EventHandler<MouseEvent>() {
					...
					public void handle(MouseEvent event) {
						if (event.getClickCount() == 2) {
							balls.clear();
							createBalls(NUM_BALLS, MIN_RADIUS, MAX_RADIUS,
									MIN_SPEED, MAX_SPEED,
									ballContainer.getWidth() / 2,
		...
		final AnimationTimer timer = new AnimationTimer() {
			...
			public void handle(long timestamp) {
				if (lastUpdateTime.get() > 0) {
					...
					checkCollisions(ballContainer.getWidth(),
	...
	private void checkCollisions(double maxX, double maxY) {
		for (ListIterator<Ball> slowIt = balls.listIterator(); slowIt.hasNext();) {
			...
			for (ListIterator<Ball> fastIt = balls.listIterator(slowIt
					.nextIndex()); fastIt.hasNext();) {
				Ball b2 = fastIt.next();
	...
	public boolean colliding(final Ball b1, final Ball b2, final double deltaX,
		...
		if (deltaX * deltaX + deltaY * deltaY <= radiusSum * radiusSum) {
			if (deltaX * (b2.getXVelocity() - b1.getXVelocity()) + deltaY
					* (b2.getYVelocity() - b1.getYVelocity()) < 0) {
				return true;
			}
		}
		return false;
	...
	private void createBalls(int numBalls, double minRadius, double maxRadius,
			double minSpeed, double maxSpeed, double initialX, double initialY) {
		...
		for (int i = 0; i < numBalls; i++) {
			...
			Ball ball = new Ball(initialX, initialY, radius,
					speed * cos(angle), speed * sin(angle), mass);
	...
	private void constrainBallsOnResize(final Pane ballContainer) {
		...
		ballContainer.heightProperty().addListener(
	...
	private static class Ball {
		...
		public Ball(double centerX, double centerY, double radius,
			...
			this.xVelocity = new SimpleDoubleProperty(this, "xVelocity",
					xVelocity);
			this.yVelocity = new SimpleDoubleProperty(this, "yVelocity",
					yVelocity);
	...
	private static class FrameStats {
		...
		private final ReadOnlyStringWrapper text = new ReadOnlyStringWrapper(
				this, "text", "Frame count: 0 Average frame interval: N/A");
		...
		public void addFrame(long frameDurationNanos) {
			meanFrameInterval = (meanFrameInterval * frameCount + frameDurationNanos / 1_000_000.0)
					/ (frameCount + 1);
			frameCount++;
			text.set(toString());
		}
		...
		public String toString() {
			return String
					.format("Frame count: %,d Average frame interval: %.3f milliseconds",
							getFrameCount(), getMeanFrameInterval());
		}
	}
